...
Phase 2 change: there is no longer a per-command dispatch table for ordinary
requests. The model reads what you said, chooses tools from tools.SCHEMAS, and
...
class Assistant:
    ...
    def load(self, interactive_setup: bool = True) -> None:
        ...
        cfg = provider.resolve(
            interactive=interactive_setup and not self.with_ui)
        self._apply_config(cfg, announce=False)
        tools_mod.init_timer_callback(self._announce)
        ...
        if self.with_ui:
            self.ui = ui_mod.UiServer(
                ...
                get_config=lambda: dict(self._current_cfg),
    ...
    def _apply_config(self, cfg: dict, announce: bool = True) -> None:
        """(Re)build the agent loop from a provider config dict."""
        self._current_cfg = cfg or {}
        client = None
        if cfg and cfg.get("provider") not in (None, "", "none"):
    ...
    def _on_ui_config_change(self, cfg: dict) -> None:
        self._apply_config(cfg, announce=True)
    ...
    def handle(self, text: str) -> None:
        ...
        with self._handle_lock:
            ...
            # Everything else goes to the model. It reads the request,
            # picks tools from tools.SCHEMAS, and calls them itself. There is
            # no per-command code here and there never will be again.
            STATE.set_activity(Activity.THINKING, "Thinking...")
            if self.agent_loop is None:
                self._speak_async(
                    "No AI model is connected. Open Settings to add a key.")
                return
            try:
                reply = self.agent_loop.handle(text)
            except Exception as exc:
                log_event("agent_error", error=str(exc)[:200])
...
def main(argv: Optional[list[str]] = None) -> int:
    parser = argparse.ArgumentParser(
        description="Ambient assistant -- voice loop, agentic tool calling, UI"
    )
